...
const CourseStructure = props => {
    const {snapshotName, courseCode, courseName, courseCredit, courseFaculty, addTeachingPeriod, appendSnapshotBySnapshotName, teachingPeriodsOrder} = props
    ...
    return (
        <div style={{
            ...
            <div style={{height: 64, marginTop: 14, width: "100%", display: "flex"}}>
                ...
                <FlatButton style={{marginLeft: "auto", marginRight: 8, zIndex: 0}} label={"COURSE INFO"}
                            labelStyle={{color: facultyColors[courseFaculty]}}/>
            </div>
            <LinearProgress color={facultyColors[courseFaculty] || "#000000"} mode={"determinate"}
                            value={courseCredit / 144 * 100} style={{height: 3}}/>
            {!snapshotName && <div>
            ...
            <FlatButton style={{fontSize: 13, borderTop: "1px solid #dddddd"}}
                        onClick={() => addTeachingPeriod(nextTeachingPeriodKey)}>
